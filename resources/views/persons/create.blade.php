<x-app-layout>
    ...
    <div x-data="{ open: true }" x-show="open" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" class="relative bg-white rounded-lg px-6 py-6 shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                <div class="text-left">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                @if($relatedPerson)
                    <li class="flex items-center">
                        ...
                        <a href="{{ route('persons.show', $relatedPerson) }}" class="text-gray-500 hover:text-gray-700 ml-1 md:ml-2">{{ $relatedPerson->full_name }}</a>
                    </li>
                @endif
                <li class="flex items-center">
                    <svg class="w-4 h-4 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <span class="text-gray-700 font-medium ml-1 md:ml-2">{{ __('Nueva persona') }}</span>
                </li>
            </ol>
        </nav>

        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('Nueva persona') }}</h1>
            @if($relatedPerson && $relation)
                <div class="mt-3 flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3">
                    @if($relatedPerson->photo_path)
                        <img src="{{ Storage::url($relatedPerson->photo_path) }}" class="w-10 h-10 rounded-full object-cover">
                    @else
                        <div class="w-10 h-10 rounded-full bg-{{ $relatedPerson->gender === 'M' ? 'blue' : 'pink' }}-100 flex items-center justify-center">
                            <span class="text-{{ $relatedPerson->gender === 'M' ? 'blue' : 'pink' }}-600 font-bold">{{ substr($relatedPerson->first_name, 0, 1) }}</span>
                        </div>
                    @endif
                    <div>
            ...
            @else
                <p class="text-gray-600 mt-1">{{ __('Agrega una nueva persona a tu arbol genealogico') }}</p>
            @endif
        </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="space-y-3">
                                @foreach($families as $family)
                                    <label class="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 {{ ($selectedFamilyId == $family->id || (!$selectedFamilyId && $loop->first)) ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
                                            ...
                                            @if($family->husband)
                                                <div class="flex items-center gap-1">
                                                    <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                                                        <span class="text-blue-600 text-sm font-medium">{{ substr($family->husband->first_name, 0, 1) }}</span>
                                                    </div>
                                                    <span class="text-sm font-medium">{{ $family->husband->full_name }}</span>
                                                </div>
                                            @else
                                                <span class="text-sm text-gray-400">{{ __('Padre desconocido') }}</span>
                                            @endif
                                            <span class="text-gray-400">&</span>
                                            @if($family->wife)
                                                <div class="flex items-center gap-1">
                                                    <div class="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center">
                                                        <span class="text-pink-600 text-sm font-medium">{{ substr($family->wife->first_name, 0, 1) }}</span>
                                                    </div>
                                                    <span class="text-sm font-medium">{{ $family->wife->full_name }}</span>
                                                </div>
                                            @else
                                                <span class="text-sm text-gray-400">{{ __('Madre desconocida') }}</span>
                                            @endif
                                        </div>
                                    </label>
                                @endforeach
