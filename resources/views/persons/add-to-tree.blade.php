    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="card">
            <div class="card-header">
                <h1 class="text-xl font-bold text-theme">{{ __('Agregar a mi árbol') }}</h1>
                <p class="text-sm text-theme-secondary mt-1">{{ __('Establece la relacion familiar con esta persona') }}</p>
            </div>
            <div class="card-body">
                <!-- Persona a agregar -->
                <div class="mb-6 p-4 bg-theme-secondary border border-theme rounded-lg">
                    <div class="flex items-center gap-4">
                        ...
                        @endif
                        <div>
                            <p class="font-semibold text-theme text-lg">{{ $person->full_name }}</p>
                ...
                <div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                    <p class="text-sm text-blue-600 dark:text-blue-400 mb-2">{{ __('Tu perfil') }}</p>
                    <div class="flex items-center gap-3">
                        @if($userPerson->photo_path)
                            <img src="{{ Storage::url($userPerson->photo_path) }}" alt="{{ $userPerson->full_name }}"
                                 class="w-12 h-12 rounded-full object-cover">
                        @else
                            <div class="w-12 h-12 rounded-full bg-blue-200 dark:bg-blue-900/40 flex items-center justify-center">
                                <span class="text-blue-600 dark:text-blue-400 font-bold">{{ substr($userPerson->first_name, 0, 1) }}</span>
                            </div>
                        @endif
                        <div>
                            <p class="font-medium text-theme">{{ $userPerson->full_name }}</p>
                        </div>
                    </div>
                </div>
                ...
                <form action="{{ route('persons.add-to-tree.store', $person) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3 mt-3">
                            <!-- Padre -->
                            <label class="relative flex items-center p-4 border rounded-lg cursor-pointer hover:bg-theme-hover transition-colors">
                ...
                @if(!$person->user_id)
                    <div class="mt-8 pt-6 border-t border-theme">
                        <p class="text-sm text-theme-muted mb-2">{{ __('¿Esta persona eres tu mismo pero con otro registro?') }}</p>
                        <a href="{{ route('persons.merge', $person) }}" class="text-sm text-theme-secondary hover:text-theme underline">
