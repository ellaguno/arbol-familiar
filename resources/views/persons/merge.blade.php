    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="card">
            <div class="card-header">
                <h1 class="text-xl font-bold text-gray-900">{{ __('Fusionar este perfil con el tuyo') }}</h1>
            </div>
            <div class="card-body">
                @if($cannotMerge)
                    ...
                    <div class="mb-6 p-4 bg-red-50 border border-red-300 rounded-lg">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <h3 class="font-semibold text-red-800">{{ __('No es posible realizar esta fusión') }}</h3>
                                <p class="text-sm text-red-700 mt-1">{{ $cannotMergeReason }}</p>
                    ...
                    <div class="grid md:grid-cols-2 gap-6 mb-6 opacity-60">
                        ...
                        <div class="p-4 bg-gray-100 border border-gray-300 rounded-lg">
                            <h3 class="text-sm font-medium text-gray-600 mb-3">{{ __('Perfil seleccionado') }}</h3>
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <div>
                                    <p class="font-semibold text-gray-700">{{ $person->full_name }}</p>
                                    @if($person->birth_date)
                                        <p class="text-sm text-gray-500">{{ $person->birth_date->format('d/m/Y') }}</p>
                                    @endif
                                    @if(!$person->is_living)
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-200 text-gray-700 mt-1">
                                            {{ __('Fallecido') }}
                                        </span>
                                    @endif
                                </div>
                        ...
                        <!-- Tu perfil -->
                        <div class="p-4 bg-gray-100 border border-gray-300 rounded-lg">
                            <h3 class="text-sm font-medium text-gray-600 mb-3">{{ __('Tu perfil') }}</h3>
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <div>
                                    <p class="font-semibold text-gray-900">{{ $userPerson->full_name }}</p>
                                    @if($userPerson->birth_date)
                                        <p class="text-sm text-gray-600">{{ $userPerson->birth_date->format('d/m/Y') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
                ...
                @else
                    ...
                    <div class="grid md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="p-4 bg-red-50 border border-red-200 rounded-lg">
                            <h3 class="text-sm font-medium text-red-800 mb-3 flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <div>
                                    <p class="font-semibold text-gray-900">{{ $person->full_name }}</p>
                                    @if($person->birth_date)
                                        <p class="text-sm text-gray-600">{{ $person->birth_date->format('d/m/Y') }}</p>
                                    @endif
                        ...
                        <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            <h3 class="text-sm font-medium text-blue-800 mb-3 flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    @if($userPerson->birth_date)
                                        <p class="text-sm text-gray-600">{{ $userPerson->birth_date->format('d/m/Y') }}</p>
                                    @endif
                                    <p class="text-xs text-gray-500">ID: {{ $userPerson->id }} ({{ __('tu perfil') }})</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            @foreach($warnings as $warning)
                                @if($warning['type'] === 'name_mismatch')
                                    ...
                                    <div class="flex items-start gap-3 p-4 bg-orange-50 border border-orange-300 rounded-lg">
                                        ...
                                        <div>
                                            <h3 class="font-semibold text-orange-800">{{ __('Advertencia: Nombres diferentes') }}</h3>
                                            <p class="text-sm text-orange-700 mt-1">{{ $warning['message'] }}</p>
                                        </div>
                                    </div>
                                @elseif($warning['type'] === 'data_loss')
                                    <!-- Advertencia de pérdida de datos -->
                                    <div class="flex items-start gap-3 p-4 bg-yellow-50 border border-yellow-300 rounded-lg">
                                        <svg class="w-6 h-6 text-yellow-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                                        </svg>
                                        <div>
                                            <h3 class="font-semibold text-yellow-800">{{ __('Advertencia: Posible pérdida de datos') }}</h3>
                    ...
                    <div class="mb-6">
                        <div class="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            ...
                            <div>
                                <h3 class="font-medium text-blue-800">{{ __('¿Qué sucede al fusionar?') }}</h3>
                    ...
                    <div class="mb-6">
                        <div class="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                            ...
                            <div>
                                <h3 class="font-medium text-amber-800">{{ __('Requiere autorización') }}</h3>
                    ...
                    <form action="{{ route('persons.merge.send', $person) }}" method="POST" class="space-y-4"
                        ...
                        @if(!$namesSimilar)
                            ...
                            <div class="flex items-start gap-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                                ...
                                <label for="confirm_names" class="text-sm text-orange-800">
                        ...
                        <div x-show="showConfirmModal" x-cloak
                            ...
                            <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 p-6"
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex-1">
                                        <h3 class="text-lg font-semibold text-gray-900">{{ __('¿Confirmar solicitud de fusión?') }}</h3>
                                        ...
                                        <ul class="text-sm text-gray-500 mt-3 space-y-1 list-disc list-inside">
