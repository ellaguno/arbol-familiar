    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="card">
            <div class="card-header">
                <h1 class="text-xl font-bold text-gray-900">{{ __('Solicitar acceso para editar familia directa') }}</h1>
                <p class="text-sm text-gray-600 mt-1">{{ __('Solicita permiso para editar la informacion de tus familiares cercanos') }}</p>
            </div>
            <div class="card-body">
                <!-- Tu perfil -->
                <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <div class="flex items-center gap-3">
                        ...
                        @else
                            <div class="w-14 h-14 rounded-full bg-blue-200 flex items-center justify-center text-blue-600 text-lg font-bold">
                                {{ substr($person->first_name, 0, 1) }}{{ substr($person->patronymic, 0, 1) }}
                            </div>
                        @endif
                        <div>
                            <p class="font-semibold text-gray-900">{{ $person->full_name }}</p>
                            <p class="text-sm text-blue-600">{{ __('Tu perfil') }}</p>
                        </div>
                    </div>
                </div>
                ...
                @if($familyToRequest->isEmpty())
                    ...
                    <div class="text-center py-8">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('Ya tienes acceso completo') }}</h3>
                        <p class="text-gray-600 mb-4">{{ __('Ya puedes editar toda tu familia directa o no tienes familiares registrados aun.') }}</p>
                ...
                @else
                    ...
                    <div class="mb-6">
                        <div class="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                            ...
                            <div>
                                <h3 class="font-medium text-amber-800">{{ __('Como funciona?') }}</h3>
                            ...
                            <div class="space-y-3">
                                @foreach($familyToRequest as $member)
                                    ...
                                    <label class="flex items-center gap-4 p-4 border rounded-lg cursor-pointer transition-colors
                                        {{ $isPending ? 'bg-gray-100 border-gray-300 cursor-not-allowed' : 'hover:bg-gray-50 border-gray-200' }}">
                                        <input type="checkbox"
                                               name="person_ids[]"
                                               value="{{ $memberPerson->id }}"
                                               {{ $isPending ? 'disabled' : '' }}
                                               class="h-5 w-5 text-[#3b82f6] border-gray-300 rounded focus:ring-[#3b82f6]">
                                        ...
                                        <div class="flex items-center gap-3 flex-1">
                                            ...
                                            @endif
                                            <div>
                                                <p class="font-medium text-gray-900">{{ $memberPerson->full_name }}</p>
                                                <p class="text-sm text-gray-500">{{ $member['label'] }}</p>
                                            </div>
                                        </div>
        ...
        @if(count($editableIds) > 0)
            <div class="card mt-6">
                <div class="card-header">
                    <h2 class="text-lg font-semibold text-gray-900">{{ __('Personas que puedes editar') }}</h2>
                </div>
                <div class="card-body">
                    <p class="text-sm text-gray-600 mb-4">{{ __('Ya tienes permiso para editar estas personas:') }}</p>
                    <div class="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                        @foreach(\App\Models\Person::whereIn('id', $editableIds)->get() as $editablePerson)
                            <a href="{{ route('persons.show', $editablePerson) }}"
                               class="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors">
                                @if($editablePerson->photo_path)
                                    <img src="{{ asset('storage/' . $editablePerson->photo_path) }}" alt="{{ $editablePerson->full_name }}"
                                         class="w-8 h-8 rounded-full object-cover">
                                @else
                                    <div class="w-8 h-8 rounded-full bg-blue-200 flex items-center justify-center text-blue-600 text-xs font-bold">
                                        {{ substr($editablePerson->first_name, 0, 1) }}
                                    </div>
                                @endif
                                <span class="text-sm font-medium text-gray-900 truncate">{{ $editablePerson->full_name }}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
