                    ...
                    @if($message->action_status === 'pending')
                        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-yellow-800 dark:text-yellow-200 font-medium">{{ __('Esta solicitud requiere tu respuesta') }}</span>
                            </div>
                    ...
                    @else
                        <div class="flex items-center gap-2">
                            @if($message->action_status === 'accepted')
                                ...
                                <span class="text-green-700 dark:text-green-300">{{ __('Aceptado el :date', ['date' => $message->action_taken_at->format('d/m/Y H:i')]) }}</span>
                            @else
                                <svg class="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                </svg>
                                <span class="text-red-700 dark:text-red-300">{{ __('Denegado el :date', ['date' => $message->action_taken_at->format('d/m/Y H:i')]) }}</span>
                            @endif
                        </div>
                    @endif
                </div>
