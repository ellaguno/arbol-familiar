...
<header class="shadow-md sticky top-0 z-50" style="background-color: var(--mf-header-bg);" x-data="{ mobileMenuOpen: false }">
    ...
    <div x-show="mobileMenuOpen"
        ...
        <div class="px-4 py-3 space-y-1">
            ...
            {{-- Chat (inyectado por plugin via hook) --}}
            @if(Route::has('chat.index'))
                <a href="{{ route('chat.index') }}"
                   class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ request()->routeIs('chat.*') ? 'text-[#EC1C24] bg-red-50 dark:bg-red-900/30' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-theme' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
