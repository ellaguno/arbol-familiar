...
<header class="bg-white shadow-md sticky top-0 z-50" x-data="{ mobileMenuOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-2">
            ...
            <div class="desktop-menu">
                ...
                <div class="flex items-center gap-1 text-xs">
                    ...
                    <nav class="flex items-center gap-1">
                        <a href="?lang=es" class="{{ app()->getLocale() == 'es' ? 'text-red-600 font-semibold' : 'text-gray-500 hover:text-red-600' }}">ESP</a>
                        <span class="text-gray-300">|</span>
                        <a href="?lang=en" class="{{ app()->getLocale() == 'en' ? 'text-red-600 font-semibold' : 'text-gray-500 hover:text-red-600' }}">ENG</a>
                    </nav>

                    @auth
                        <span class="text-gray-300 mx-1">|</span>
                        <!-- Icono Home -->
                        <a href="{{ route('dashboard') }}" class="mx-1 px-2 text-gray-500 hover:text-[#3b82f6]" title="{{ __('Inicio') }}">
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="mx-1 inline">
                            @csrf
                            <button type="submit" class="text-gray-500 hover:text-red-600 font-medium">
                                {{ __('SALIR') }}
                            </button>
                        </form>
                    @else
                        <span class="text-gray-300 mx-1">|</span>
                        <a href="{{ route('login') }}" class="text-gray-500 hover:text-[#3b82f6] font-medium">
                            {{ __('INGRESAR') }}
                        </a>
                    @endauth
                </div>
                ...
                @auth
                    ...
                    <nav class="flex items-center gap-1 text-[13px] whitespace-nowrap mt-1" style="line-height: 21.31px;">
                        ...
                        <span class="text-gray-300">|</span>
                        <span class="px-2 py-0.5 text-gray-400 cursor-not-allowed relative group" title="{{ __('Próximamente') }}">
                            {{ __('INVESTIGACIÓN') }}
            ...
            <button @click="mobileMenuOpen = !mobileMenuOpen"
                    class="mobile-hamburger p-2 rounded-md text-gray-600 hover:text-[#3b82f6] hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-[#3b82f6]"
            ...
            <div class="mobile-hamburger flex items-center gap-2 text-xs">
                <nav class="flex items-center gap-1">
                    <a href="?lang=es" class="{{ app()->getLocale() == 'es' ? 'text-red-600 font-semibold' : 'text-gray-500 hover:text-red-600' }}">ES</a>
                    <span class="text-gray-300">|</span>
                    <a href="?lang=en" class="{{ app()->getLocale() == 'en' ? 'text-red-600 font-semibold' : 'text-gray-500 hover:text-red-600' }}">EN</a>
                </nav>
                <span class="text-gray-300">|</span>
                <a href="{{ route('login') }}" class="text-gray-500 hover:text-[#3b82f6] font-medium">
                    {{ __('INGRESAR') }}
    ...
    <div x-show="mobileMenuOpen"
         ...
         class="mobile-menu border-t border-gray-200 bg-gray-50"
         x-cloak>
        <div class="px-4 py-3 space-y-1">
            <!-- Navegación principal -->
            <a href="{{ route('tree.index') }}"
               class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ request()->routeIs('tree.*') ? 'text-[#EC1C24] bg-red-50' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-gray-100' }}">
            ...
            <a href="{{ route('messages.inbox') }}"
               class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ request()->routeIs('messages.*') ? 'text-[#EC1C24] bg-red-50' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-gray-100' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if(auth()->user()->person)
                <a href="{{ route('persons.show', auth()->user()->person) }}"
                   class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ request()->routeIs('profile.*') || (request()->routeIs('persons.show') && request()->route('person')?->id == auth()->user()->person_id) ? 'text-[#EC1C24] bg-red-50' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <a href="{{ route('profile.edit') }}"
                   class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ request()->routeIs('profile.*') ? 'text-[#EC1C24] bg-red-50' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                    </svg>
                    {{ __('Perfil') }}
            ...
            <a href="{{ route('search.index') }}"
               class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ request()->routeIs('search.*') ? 'text-[#EC1C24] bg-red-50' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-gray-100' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('persons.index') }}"
               class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ $mobileIsCommunityActive ? 'text-[#EC1C24] bg-red-50' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-gray-100' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <span class="flex items-center px-3 py-2 rounded-md text-sm font-medium text-gray-400 cursor-not-allowed">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                {{ __('Investigación') }}
                <span class="ml-2 text-xs text-gray-400">({{ __('Próximamente') }})</span>
            </span>
            ...
            <div x-data="{ langOpen: false }" class="relative">
                <button @click="langOpen = !langOpen"
                        class="flex items-center justify-between w-full px-3 py-2 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-200">
                ...
                <div x-show="langOpen" x-transition class="pl-8 mt-1 space-y-1">
                    <a href="?lang=es" class="block px-3 py-2 rounded-md text-sm {{ app()->getLocale() == 'es' ? 'bg-red-100 text-red-700 font-semibold' : 'text-gray-600 hover:bg-gray-200' }}">
                        Español
                    </a>
                    <a href="?lang=en" class="block px-3 py-2 rounded-md text-sm {{ app()->getLocale() == 'en' ? 'bg-red-100 text-red-700 font-semibold' : 'text-gray-600 hover:bg-gray-200' }}">
                        English
                    </a>
                </div>
            </div>
            ...
            <!-- Ayuda -->
            <a href="{{ route('help') }}"
               class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ request()->routeIs('help') ? 'text-[#EC1C24] bg-red-50' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-gray-100' }}">
                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if(auth()->user()->is_admin)
                <a href="{{ route('admin.index') }}"
                   class="flex items-center px-3 py-2 rounded-md text-[13px] font-medium {{ request()->routeIs('admin.*') ? 'text-[#EC1C24] bg-red-50' : 'text-[#8896C9] hover:text-[#EC1C24] hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
