...
<body class="font-sans antialiased bg-theme">
    <div class="min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            ...
            <h2 class="mt-6 text-center text-3xl font-extrabold text-theme">
                {{ __('Invitacion de consentimiento') }}
            </h2>
        </div>

        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-lg">
            <div class="bg-theme-card py-8 px-4 shadow sm:rounded-lg sm:px-10">
                @if(session('error'))
                ...
                <div class="mb-6">
                    ...
                    <p class="text-center text-theme-secondary mb-4">
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-4">
                        <h3 class="font-semibold text-blue-900 dark:text-blue-300 mb-2">{{ __('Tu informacion registrada:') }}</h3>
                ...
                @if($existingUser)
                    ...
                    <div class="flex gap-4">
                        ...
                        <form action="{{ route('invitation.decline', $invitation->token) }}" method="POST" class="flex-1">
                            @csrf
                            <button type="submit" class="w-full btn-outline text-red-600 dark:text-red-400 border-red-300 dark:border-red-700 hover:bg-red-50 dark:hover:bg-red-900/20">
                                {{ __('Rechazar') }}
                            </button>
                        </form>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="email" id="email" value="{{ $invitation->email }}"
                                       class="form-input bg-theme-secondary" readonly>
                            </div>
                            ...
                            <div class="flex items-start">
                                <input type="checkbox" name="terms" id="terms"
                                       class="h-4 w-4 text-blue-600 border-theme rounded mt-1 @error('terms') border-red-500 @enderror" required>
                                <label for="terms" class="ml-2 text-sm text-theme-secondary">
                                    {{ __('Acepto los') }}
                                    <a href="{{ route('terms') }}" target="_blank" class="text-blue-600 dark:text-blue-400 hover:underline">{{ __('terminos y condiciones') }}</a>
                                    {{ __('y la') }}
                                    <a href="{{ route('privacy') }}" target="_blank" class="text-blue-600 dark:text-blue-400 hover:underline">{{ __('politica de privacidad') }}</a>
                                </label>
                            </div>
                            @error('terms')
                                <p class="form-error">{{ $message }}</p>
